...
/**
 * Checks for Duck app updates against this repository's GitHub Releases.
 *
 * Used in two places:
 *  - a silent check on every app launch (DuckRoot) that pops a dialog when a
 *    newer version is published, and
 *  - a manual "Check for updates" action in the Settings sheet.
 *
 * Version discovery deliberately avoids the rate-limited REST API as its
 * primary strategy, mirroring the hard-earned lesson in [com.devfahim00.duck.ytdlp.YtDlpUpdater]:
 * GitHub allows only 60 anonymous API requests/hour *per source IP*, and on
 * carrier mobile networks (very common for our users, heavy CGNAT) that quota
 * is shared by huge numbers of subscribers and dies almost instantly. So:
 *
 ...
 * `Duck-arm64-v8a-<tag>.apk` (see .github/workflows/android.yml), so even when
 * only the redirect trick succeeds we can construct a direct download link.
 */
object AppUpdater {

    private const val OWNER = "devfahim00"
    private const val REPO = "Duck"

    /** Telegram of the owner - surfaced in Settings. */
    ...
    private fun apkUrlFor(tag: String): String =
        "https://github.com/$OWNER/$REPO/releases/download/$tag/Duck-arm64-v8a-$tag.apk"
